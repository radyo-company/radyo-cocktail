...
public class CocktailAddStepCommand : CocktailIntegrationTests
{
    [Fact]
    public async Task ShouldHaveANewStep()
    {
        // Arrange
        ...
        negroni.AddStep(new Step(2, "Garnish with orange twist."));
        ...
        var stepName = "Shake really really hard";
        
        var request =
            new Cocktail.Application.Handlers.Commands.CocktailAddStepCommand(negroni.Id, stepName);
        ...
        cocktail.Steps.Any(x => x.Description == stepName && x.Order == 3).Should().BeTrue();
